...
deploy: build/
	...
	aws s3 sync public/ s3://$(s3_bucket_name) --cache-control no-cache --acl public-read --metadata-directive REPLACE --include "*" --exclude "*.html" --exclude "*.txt" --exclude "*.css" --exclude "*.xml" --exclude "*.svg" --exclude "*.jpg" --exclude "*.tiff" --exclude "*.aif" --exclude "*.aiff" --exclude "*.mp3" --exclude "*.wav" --profile mach1
